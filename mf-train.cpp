...
Option parse_option(int argc, char **argv)
{
    ...
    for(i = 1; i < argc; i++)
    {
        ...
        {
            if((i+1) >= argc)
                throw invalid_argument("need to specify rho after -r");
            i++;
            option.param.rho = atof(argv[i]);
            if(option.param.rho <= 0)
                throw invalid_argument("rho should be greater than zero");
        }
        ...
        {
            if((i+1) >= argc)
                throw invalid_argument("need to specify epsilon after -e");
            i++;
            option.param.epsilon = atof(argv[i]);
            if(option.param.epsilon <= 0)
                throw invalid_argument("epsilon should be greater than zero");
        }
